...
Options:
  ...
  --max-parallel N        Concurrent (target, objective) MILP solves (default: 1).
                          Each solve is single-threaded; keep N <= --cpus.
...
# Each MILP solve is single-threaded; with --max-parallel > 1 the per-process
# BLAS/XLA pools must not multiply, or the node oversubscribes.
...
echo "Max parallel MILP solves: ${MAX_PARALLEL}"
...
PIPELINE_CMD=("\$PYTHON_BIN" scripts/run_alphaq_external_validation_pipeline.py \\
  ...
  --max-parallel $(printf "%q" "$MAX_PARALLEL") \\
